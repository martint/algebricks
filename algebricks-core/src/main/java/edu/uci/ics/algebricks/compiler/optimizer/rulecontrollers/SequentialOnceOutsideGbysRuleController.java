...
public class SequentialOnceOutsideGbysRuleController extends AbstractRuleController {

    public SequentialOnceOutsideGbysRuleController(IOptimizationContext context) {
        super(context);
    }
    ...
    public boolean rewriteWithRuleCollection(LogicalOperatorReference root, Collection<IAlgebraicRewriteRule> rules)
        ...
        for (IAlgebraicRewriteRule rule : rules) {
            if (rewriteOperatorRef(root, rule, false, false)) {
                fired = true;
            }
        }
        return fired;
